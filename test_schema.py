...
"""
from schema import (
    Character, Plotline, Location, LoreEntry, POVState, ChapterSummary,
    RetrievalConfig, ContextPack, DependencyGraphHit,
...
def test_basic_models():
    ...
    lore = LoreEntry(category="magic_system", title="Bloodbinding", content="Magic requires a blood price")
    ...
    print("Basic models OK:", alice.name, plot.name, loc.name, lore.title, pov.location_id, summary.chapter_number)
    return alice, bob, plot, loc, lore, pov, summary


def test_context_pack(alice, bob, plot, loc, lore, pov, summary):
    pack = ContextPack(
        pov_state=pov,
...
def test_discriminated_union(alice, plot, loc):
    ...
    # Round-trip through the actual union type used in ChapterGraphState to prove discrimination works
...
def test_graph_state(alice, bob, plot, loc, lore, pov, summary, pack, plan, reasoning, patches):
    ...
    # simulate the retry-then-flag path
    ...
    # round-trip the whole state through JSON to make sure nested union serializes/deserializes
...
if __name__ == "__main__":
    alice, bob, plot, loc, lore, pov, summary = test_basic_models()
    pack = test_context_pack(alice, bob, plot, loc, lore, pov, summary)
    plan, reasoning = test_plan_and_reasoning(alice, bob, plot)
    test_canon_check()
    patches = test_discriminated_union(alice, plot, loc)
    test_graph_state(alice, bob, plot, loc, lore, pov, summary, pack, plan, reasoning, patches)
